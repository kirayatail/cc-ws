...
const updateClient = (data) => {
  const foundIndex = clients.findIndex((c) => c.id === data.id);
  if (foundIndex > -1) {
    if (data.type === "all") {
      clients[foundIndex] = {
        ...clients[foundIndex],
      ...
      // console.log(`Setting ${data.type} for client ${data.id}`);
      clients[foundIndex][data.type] = data.payload;
    }
    clients
      ...
      .forEach((c) =>
        c.sock.send(
          JSON.stringify({
            type: "info",
            id: data.id,
            payload: clientToSend(clients[foundIndex]),
          })
        )
      );
  }
  ...
  wss.on("connection", (ws, req) => {
    ...
    ws.on("message", (msg) => {
      ...
      if (data.type === "handshake") {
        const id = clients
          .map((c) => c.id)
          .reduce((cand, c) => (cand > c ? cand : c + 1), 0);
